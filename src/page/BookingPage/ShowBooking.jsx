...
//   const [selectedCab, setSelectedCab] = useState(null);
  ...
  return (
    <div className="min-h-screen bg-white">
      <Header />
      {/* Hero Section */}
      <div className="relative pt-20 pb-8 px-4 mt-15 bg-green-900">
        <div className="container mx-auto text-center">
          <h1 className="text-4xl font-bold mb-3 text-white">
            Find Your Perfect Ride
          </h1>
          <p className="text-yellow-100 text-base mb-6 max-w-2xl mx-auto">
            Choose from our wide selection of premium vehicles for your journey
          </p>
          {/* Search Bar */}
          <div className="max-w-xl mx-auto relative">
            <div className="relative">
              <Search className="absolute left-4 top-3 h-4 w-4 text-green-600" />
              <input
                type="text"
                placeholder="Where would you like to go?"
                className="w-full pl-10 pr-4 py-2.5 rounded-full border border-green-700 focus:ring-2 focus:ring-yellow-400 focus:border-transparent outline-none transition-all text-sm"
              />
            </div>
          </div>
        </div>
        ...
        <div className="bg-green-50 p-4 rounded-xl shadow-sm border border-green-100">
          <div className="flex flex-wrap gap-4 justify-center items-center">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-sm font-semibold text-green-900">Filters</h3>
            </div>
            {["category", "available", "location"].map((filterType) => (
              <select
                ...
                className="bg-white border border-green-200 p-2 rounded-lg focus:ring-2 focus:ring-yellow-400 focus:border-transparent outline-none transition-all min-w-[140px] text-sm text-green-800"
              >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredCars.length > 0 ? (
            filteredCars.map((car) => (
              <div
                key={car.carId}
                className="group bg-white rounded-xl shadow-sm border border-green-100 transition-all duration-300 hover:shadow-md hover:border-yellow-200"
              >
                <div className="relative">
                  <img
                    src={car.carImageUrl}
                ...
                <div className="p-3">
                  ...
                  <div className="space-y-1 mb-3">
                    <p className="text-xs text-green-600 flex items-center justify-between">
                      <span>Category:</span>
                  ...
                  <button
                    ...
                    className={`w-full py-2 rounded-lg text-sm font-medium transition-all ${car.available ? "bg-yellow-400 text-green-900 hover:bg-yellow-500" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}
                  >
